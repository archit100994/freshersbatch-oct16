...
public class Nqueen
{
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException
	{
		...
		Callable<Integer[]> c[] = new Callable[8];
		{
			for(int a=0; a<8; a++)
			{
				final int final_a = a;
				c[a]= new Callable<Integer[]>() 
				{
...
//						int i = 1;
										...
										for(int f=0; f<=7; f++) 
											for(int g=0; g<=7; g++) 
												for(int h=0; h<=7; h++) 
													...
													if(isSafePosition(final_a, b, a2, b2) && isSafePosition(final_a, c, a2, c2) && isSafePosition(final_a, d, a2, d2) && isSafePosition(final_a, e, a2, e2) && isSafePosition(final_a, f, a2, f2) && isSafePosition(final_a, g, a2, g2) && isSafePosition(final_a, h, a2, h2)
															...
															System.out.println("Position " + i + ") " + final_a + " - " + b + " - " + c + " - " + d + " - " + e + " - " + f + " - " + g + " - " + h);
													i++;}
		...
		Future<Integer[]> future[]= new Future[8];
		for(int in=0; in<8; in++)
		{			
			future[in]=ex.submit(c[in]);
//			future.get();
		}
		ex.shutdown();
		for(int in=0; in<8; in++)
